...
const postSuccessfulStep = async (results, originalURL) => {
  ...
  await asyncForEach(results, async ({
    ...
    if (isSaveLocal && dirHandle && (docx || html || md || xml)) {
      ...
      if (config.fields['import-local-xml'] && md) {
        files.push({ type: 'xml', filename: `${path}.xml`, data: xml });
      }
...
const attachListeners = () => {
  ...
  IMPORT_BUTTON.addEventListener('click', (async () => {
    ...
    isSaveLocal = config.fields['import-local-docx'] || config.fields['import-local-html'] || config.fields['import-local-md'] || config.fields['import-local-xml'];
