...
async function html2x(
  ...
  doc,
  transformCfg,
  config = { toMd: true, toDocx: false, toXml: false },
  params = {},
) {
  ...
  const importer = new InternalImporter({
    ...
    skipXmlFileCreation: !config.toXml,
    logger,
    mdast2docxOptions: {
  ...
  const getResponseObjectFromPIR = async (pir) => {
    ...
    // TODO removed config.toXml part for now, fix later
    if (pir.xml) {
...
async function html2xml(url, document, transformCfg, config, params = {}) {
  let doc = document;
  if (typeof doc === 'string') {
    doc = parseStringDocument(document, config);
  }
