...
export default class PageImporter {
  ...
  async convertToDocx(docxPath, content) {
    const buffer = await md2docx(content, {
      log: this.logger,
      ...this.params.mdast2XmlOptions,
    });
    return this.params.storageHandler.put(docxPath, buffer);
  }

  async convertToXml(xmlPath, content) {
    const buffer = await md2xml(content, {
      log: this.logger,
      ...this.params.xast2xmlOptions,
    });
    return this.params.storageHandler.put(xmlPath, buffer);
  }
  ...
  async createXml(resource, url) {
      console.log("createXml has been called... 123");
      const { name } = resource;
      const { directory } = resource;
      const sanitizedName = FileUtils.sanitizeFilename(name);
      ...
      if (entries) {
        await Utils.asyncForEach(entries, async (entry) => {
          if (entry.document) {
            ...
            if (!this.params.skipXmlConversion) {
              const xmlPath = `${res.path}.xml`;
              await this.convertToXml(xmlPath, res.hast);
              // eslint-disable-next-line no-param-reassign
              entry.xml = xmlPath;
            }
